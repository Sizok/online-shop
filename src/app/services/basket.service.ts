...
export class BasketService {
  ...
  removeQuantity(item: CartItem){
    ...
    this._snackBar.open('1 товар був видалений із кошика', 'Ok', {duration: 3000})
  }
  ...
  removeFromBasket(item: CartItem, update = true):Array<CartItem> {
    const filteredItems = this.cart.value.items.filter((_item)=> _item.id !== item.id)
    if(update){
      this.cart.next({items: filteredItems});
      this._snackBar.open('1 один товар видалений із кошика', 'Ok', {duration:3000});
    }
